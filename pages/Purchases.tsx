...
import { Search, Plus, Minus, Trash2, Truck, X, Check, Save, FileText, Calendar, Filter, UserPlus, Phone, CreditCard, Eye, Package, Barcode, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const Purchases: React.FC<PurchasesProps> = ({ exchangeRate }) => {
  ...
  const totalPages = Math.max(1, Math.ceil(purchaseReport.purchases.length / PAGE_SIZE));
  const paginatedPurchases = purchaseReport.purchases.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {paginatedPurchases.length === 0 ? (
                                ...
                                paginatedPurchases.map(purch => {
                                  ...
                                  return (
                                      <tr key={purch.id} className="hover:bg-gray-50">
                                          <td className="px-6 py-4 text-sm text-gray-600">
                                              {new Date(purch.date).toLocaleString()}
                                              <div className="text-xs text-gray-400 mt-1">{purch.id}</div>
                                          </td>
                  ...
                  {totalPages > 1 && (
                      <div className="flex items-center justify-between px-5 py-4 border-t border-gray-100 bg-gray-50">
                          <span className="text-sm text-gray-500">
                              Mostrando {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, purchaseReport.purchases.length)} de {purchaseReport.purchases.length}
                          </span>
                          <div className="flex items-center gap-2">
                              <button
