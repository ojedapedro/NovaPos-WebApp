  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchTerm.trim()) {
        ...
        if (exactMatch) {
            ...
            handleEditProduct(exactMatch); // Open edit modal automatically
            showNotification('info', `Editando: ${exactMatch.name}`);
        }
        // If no exact match, the list is already filtered by the generic search logic, which is fine for partials.
    }
  };
        ...
        <div className="p-4 border-b border-gray-100 space-y-4">
            <div className="flex gap-4">
                <div className="relative flex-1">
                    ...
                    className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
                    value={searchTerm}
