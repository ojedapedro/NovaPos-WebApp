...
export const POS: React.FC<POSProps> = ({ exchangeRate, onUpdateExchangeRate }) => {
  ...
  const processSale = () => {
    ...
    // DB-03 FIX: crypto.randomUUID() evita colisiones de ID
    const saleId = `V${crypto.randomUUID().split('-')[0].toUpperCase()}`;
    ...
    // FUNC-02 FIX: Cashea ahora genera movimiento de caja correctamente
    if (paymentDetails[PaymentMethod.CASHEA] > 0) movements.push(createMovement(saleId, PaymentMethod.CASHEA, paymentDetails[PaymentMethod.CASHEA], 'USD'));
  ...
  // UX-07 FIX: .substring() reemplaza .substr() deprecado
  const createMovement = (refId: string, method: PaymentMethod, amount: number, currency: string) => ({
    id: `M${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
    date: new Date().toISOString(),
  ...
  const salesReport = useMemo(() => {
    ...
    return {
        // FUNC-03 FIX: spread antes de reverse() para no mutar el array original
        sales: [...filtered].reverse(),
        totalRevenueUSD,
        count: filtered.length
    };
  }, [salesHistory, startDate, endDate, filterClient]);

  const totalPages = Math.max(1, Math.ceil(salesReport.sales.length / PAGE_SIZE));
  const paginatedSales = salesReport.sales.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between px-5 py-4 border-t border-gray-100 bg-gray-50">
                        <span className="text-sm text-gray-500">
                            Mostrando {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, salesReport.sales.length)} de {salesReport.sales.length}
                        </span>
                        <div className="flex items-center gap-2">
                            <button
