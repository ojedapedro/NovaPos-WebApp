// =====================================================
// NovaPOS — Google Apps Script Backend
// =====================================================
// SEGURIDAD: La URL de este script y el SPREADSHEET_ID
// no deben compartirse públicamente.
// El token de autenticación se configura en:
//   Apps Script -> Proyecto -> Propiedades del script -> API_TOKEN
// =====================================================

const SPREADSHEET_ID = '1GPTjlwwFtvsjGDY8sgEbo2UCemwbu1MuZwPgnnpeinM';

/**
 * Valida que la petición incluya el token correcto.
 * El token se guarda en Script Properties (no en el código).
 */
function isAuthorized(request) {
  const storedToken = PropertiesService.getScriptProperties().getProperty('API_TOKEN');
  // Si no hay token configurado, permitir (modo desarrollo).
  // En producción, SIEMPRE configura el token.
  if (!storedToken || storedToken === '') {
    Logger.log('ADVERTENCIA: API_TOKEN no configurado. Cualquier petición es aceptada.');
    return true;
  }
  return request.token === storedToken;
}
...
/**
 * API: DO GET
 * Devuelve todos los datos para inicializar la app.
 */
function doGet(e) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
...
/**
 * API: DO POST
 * Maneja la escritura de datos con validación de token (SEC-04).
 */
function doPost(e) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  ...
  try {
    ...
    return errorResponse('Formato de petición inválido');
  }

  // --- Validación de autenticación (SEC-04) ---
  if (!isAuthorized(request)) {
    Logger.log('Petición rechazada: token inválido');
  ...
  try {
    switch (action) {
      ...
      case 'DELETE_SUPPLIER':
        ...
        break;
      // FUNC-01 FIX: SAVE_CLIENT ahora existe en el backend
      case 'SAVE_CLIENT':
        saveClient(ss, request.payload);
        break;
      case 'SAVE_MOVEMENT':
        saveMovement(ss, request.payload);
        break;
      default:
        result = { status: 'error', message: 'Acción desconocida: ' + action };
    }
  } catch (error) {
    Logger.log('Error en acción ' + action + ': ' + error.toString());
...
function updateStock(ss, items, isPurchase) {
  ...
  items.forEach(item => {
    ...
    if (idMap.has(productId)) {
      ...
      const currentStock = Number(data[rowIndex][5]) || 0;
...
function saveSupplier(ss, supplier) {
    ...
    if (!found) {
        sheet.appendRow([supplier.id, supplier.name, supplier.phone, supplier.paymentType]);
    }
}
...
// FUNC-01 FIX: Función saveClient que faltaba en el backend
function saveClient(ss, client) {
    ...
    for (let i = 1; i < data.length; i++) {
        if (data[i][0] == client.id) {
            sheet.getRange(i + 1, 1, 1, 4).setValues([[
                client.id, client.name, client.phone, client.type
            ]]);
            found = true;
            break;
        }
    ...
    if (!found) {
        sheet.appendRow([client.id, client.name, client.phone, client.type]);
    }
}

// FUNC-06 FIX: Función para guardar movimientos manuales de caja
function saveMovement(ss, movement) {
    const sheet = ss.getSheetByName('Movimientos_Caja');
    sheet.appendRow([
        movement.id, movement.date, movement.type, movement.origin,
        movement.method, movement.amount, movement.currency,
        movement.reference || '', movement.supplierId || ''
    ]);
}